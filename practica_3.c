...
#use fast_io(c)
#use fast_io(d)
int contador100ms=0;         //Declaramos variable contador para tiempo de 100ms. Sabremos cuantas veces se ha ejecutado la interrupción
int contador500ms=0;         //Declaramos variable contador para tiempo de 100ms. Sabremos cuantas veces se ha ejecutado la interrupción
int contador1000ms=0;        //Declaramos variable contador para tiempo de 100ms. Sabremos cuantas veces se ha ejecutado la interrupción
int contador1500ms=0;        //Declaramos variable contador para tiempo de 100ms. Sabremos cuantas veces se ha ejecutado la interrupción
...
void main(void)
{
   ...
   {
      if(contador100ms>=6)   //Si nuestro contador_100ms es mayor o igual a 6 quiere decir que ya han pasado 100ms desde que se desplazo conAnilloA entonces, se ejecuta contador de anillo del puerto A
      {
         conAnilloA>>=1;            //Desplazamos el bit del contador de anilo del puerto A
         contador100ms=0;          //Regresemos el valor de contador_100ms a 0
         if(conAnilloA==0)          //Si el valor de conAnilloA sobrepaso el LSM, devuelbe el valor al MSB
         {
            conAnilloA=0x20; 
         }
      }
      if(contador500ms>=30)   //Si nuestro contador_500ms es mayor o igual a 30 quiere decir que ya han pasado 500ms desde que se desplazo conAnilloB entonces, se ejecuta contador de anillo del puerto B
      {
         conAnilloB>>=1;            //Desplazamos el bit del contador de anillo del puerto B
         contador500ms=0;          //Regresemos el valor de contador_500ms a 0
         if(conAnilloB==0)          //Si el valor de conAnilloB sobrepaso el LSM, devuelbe el valor al MSB
         {
            conAnilloB=0x80;
         }
      }
      if(contador1000ms>=61)   //Si nuestro contador_1000ms es mayor o igual a 61 quiere decir que ya han pasado 1000ms desde que se desplazo conAnilloC entonces, se ejecuta contador de anillo del puerto C
      {
         conAnilloC>>=1;            //Desplazamos el bit del contador de anilo del puerto C
         contador1000ms=0;         //Regresemos el valor de contador_1000ms a 0
         if(conAnilloC==0)          //Si el valor de conAnilloC sobrepaso el LSM, devuelbe el valor al MSB
         {
            conAnilloC=0x80; 
         }
      }
      if(contador1500ms>=91)   //Si nuestro contador_1500ms es mayor o igual a 91 quiere decir que ya han pasado 1500ms desde que se desplazo conAnilloD entonces, se ejecuta contador de anillo del puerto D
      {
         conAnilloD>>=1;            //Desplazamos el bit del contador de anilo del puerto D
         contador1500ms=0;         //Regresemos el valor de contador_1500ms a 0
